...
export const FIRE_ALARM_WEEKLY_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'Tests one call point per week in rotation. BS 5839-1 requires all points tested over a cycle, not all weekly.',
...
export const EMERGENCY_LIGHTING_MONTHLY_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'If you have 10+ lights, rotate monthly tests. Each unit tested once per month maintains compliance.',
...
export const FIRE_DOOR_CHECK_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'If you have 10+ fire doors, rotate monthly checks. Each door checked once per month maintains compliance.',
...
export const EXTINGUISHER_MONTHLY_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'Quick visual check of all extinguishers monthly. Annual service by competent person required separately.',
...
export const SPRINKLER_WEEKLY_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'System-wide weekly check. Quarterly service by competent person required separately.',
...
export const FALSE_ALARM_LOG_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'Recording log for false alarms as they occur. Used for trend analysis and compliance monitoring.',
...
export const TRAINING_DRILL_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'Recording log for all training and drills. Fire Safety Order Article 21 requires relevant persons receive fire safety training. Evacuation drills recommended at least twice yearly.',
...
export const DETECTOR_INSPECTION_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'Visual inspection of detectors in rotation. If you have 10+ detectors, rotating monthly inspections ensures all are checked over time.',
...
export const DETECTOR_PUSH_TEST_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  name: 'Standalone Detector – Weekly Push Button Test',
  description: 'Weekly functional test of standalone and linked smoke/heat/CO detectors',
  assetType: 'smoke_detector',
  frequency: 'weekly',
  rrule: 'FREQ=WEEKLY;BYDAY=MO',
  strategy: 'rotate',
  ...
  requiresGPS: true,
  requiresSignature: true,
  guidance: 'Press and hold the test button on the detector. Alarm should sound within a few seconds. If linked, all interconnected detectors should also sound.',
  complianceNote: 'Weekly push button test in rotation. BS 5839-6 requires all standalone/linked detectors tested over a cycle to ensure operational functionality.',
  references: ['BS 5839-6:2019'],
  version: 1,
  fields: [
    {
      ...
      type: 'text',
      required: true,
      guidance: 'Specify room or area where detector is located',
    },
    {
      id: 'test_button_pressed',
      label: 'Test button pressed and held',
      type: 'boolean',
      required: true,
    },
    {
      ...
      type: 'enum',
      required: true,
      options: ['Yes - Loud', 'Yes - Quiet/Weak', 'No'],
      guidance: 'Alarm should sound clearly within a few seconds',
    },
    {
      id: 'linked_alarms_activated',
      label: 'Linked alarms activated (if applicable)',
      type: 'enum',
      required: true,
      options: ['All activated', 'Some failed', 'Not linked', 'N/A'],
      guidance: 'If detectors are interconnected, all should sound',
    },
    {
      ...
      type: 'enum',
      required: true,
      options: ['Good', 'Low battery warning', 'Not visible', 'Mains powered'],
      guidance: 'Check for low battery chirp or indicator light',
    },
    {
      id: 'result',
      label: 'Overall result',
      ...
      required: true,
      options: ['Pass', 'Fail'],
      guidance: 'FAIL if alarm did not sound or linked detectors failed to activate',
    },
    {
      id: 'actions',
      label: 'Actions required',
      type: 'text',
      required: false,
      guidance: 'Note battery replacement, repair, or replacement needed',
    },
  ],
};
...
export const SOUNDER_STROBE_TEST_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'During weekly alarm test, verify sounders/strobes in rotation. BS 5839-1 requires all devices tested over a cycle, not all weekly.',
...
export const FINAL_EXIT_DOOR_CHECK_TEMPLATE: Omit<CheckTemplate, 'id' | 'orgId' | 'createdAt' | 'updatedAt'> = {
  ...
  complianceNote: 'Weekly check of all final exit doors. Fire Safety Order Article 14 requires means of escape maintained and free from obstruction. All exits must be checked during occupied hours.',
